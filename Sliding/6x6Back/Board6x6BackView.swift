...
class Board6x6BackView : UIView{
    ...
    override func layoutSubviews() {
        ...
        let board = appDelegate.board6x6Back  // get model from app delegate
        
        let boardSquare = boardRect()  // determine region to hold tiles (see below)
        let tileSize = (boardSquare.width) / 6.0
        let tileBounds = CGRect(x: 0, y: 0, width: tileSize, height: tileSize)
        
        for r in 0 ..< 6 {      // manually set the bounds, and of each tile
            for c in 0 ..< 6 {
