...
class ViewController5x5Back: UIViewController {
    ...
    @IBOutlet weak var boardView: Board5x5BackView!
    ...
    @IBAction func tileSelected(_ sender: UIButton) {
        ...
        let board = appDelegate.board5x5Back
        ...
        if slide {
            ...
            if (board!.isSolved()) {
                CheckRecors().Check(move: moveCountPlus, time: timeLabel.text ?? "", levelSquare: "5x5")
                if(userDefaults.string(forKey: "name") != ""){
                    //AddFirebase().addMoveFirebase(square: "5x5", move: moveCountPlus)
                }
                level += 1
                coin += 10
    ...
    @IBAction func moveOverUseCoin(_ sender: Any) {
        if(coin >= 50){
            ...
            let dialogMessage = UIAlertController(title: "COINS ARE NOT ENOUGH", message: "You must have at least 50 coins", preferredStyle: .alert)
   ...
   func adDidDismissFullScreenContent(_ ad: GADFullScreenPresentingAd) {
    ...
    private func nextLevelAds(){
        if(level == 25){
            ...
            AudioPlayerBackground.stop()
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            let vc = storyboard.instantiateViewController(withIdentifier: "6x6VC")
            self.present(vc, animated: true)
        }
    }
