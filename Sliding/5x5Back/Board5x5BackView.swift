...
class Board5x5BackView : UIView{
    ...
    override func layoutSubviews() {
        ...
        let board = appDelegate.board5x5Back  // get model from app delegate
        
        let boardSquare = boardRect()  // determine region to hold tiles (see below)
        let tileSize = (boardSquare.width) / 5.0
        let tileBounds = CGRect(x: 0, y: 0, width: tileSize, height: tileSize)
        
        for r in 0 ..< 5 {      // manually set the bounds, and of each tile
            for c in 0 ..< 5 {
                let tile = board!.getTile(atRow: r, atColumn: c)
                if tile > 0 {
                    let buttonsView = self.viewWithTag(26)
                    let button = self.viewWithTag(tile) as! UIButton
                    button.translatesAutoresizingMaskIntoConstraints = false
                    button.layer.cornerRadius = 10
                    button.layoutMargins = UIEdgeInsets(top: 10, left: 5, bottom: 5, right: 5)
            ...
            for c in 0..<5 {
                ...
                if tile > 0 {
                    ...
                    if (imageOn) {
                        ...
                        button.titleEdgeInsets = UIEdgeInsets(top: 5, left: 5, bottom: 5, right: 5)// no margins
